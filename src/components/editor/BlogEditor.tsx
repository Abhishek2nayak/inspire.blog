...
import ScheduledPostModal from "./editorcomponents/ScheduledPostModal";
import { EDITOR_LANGUAGES, SLASH_COMMANDS } from "@/lib/consts";
import { inlineToMd, markdownToHtml, nodeToMd } from "@/lib/editor.utils";
...
export default function BlogEditor({
  ...
  const editor = useEditor({
    ...
    onUpdate({ editor }) {
      ...
      // Auto-save draft
      if (!postId)
        triggerAutoSave(title, subtitle, editor.getJSON() as unknown as string);
    },
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (draft.title || draft.subtitle || draft.content)
          setDraftBanner(true);
      }
    } catch {
      /* ignore */
  ...
  const triggerAutoSave = useCallback(
    (t: string, s: string, content: string) => {
      if (autoSaveTimer.current) clearTimeout(autoSaveTimer.current);
      autoSaveTimer.current = setTimeout(() => {
        try {
          localStorage.setItem(
            DRAFT_KEY,
            JSON.stringify({ title: t, subtitle: s, content }),
          );
        } catch {
          /* ignore */
        }
    ...
    (cmd: SlashCommand) => {
      ...
      if (matchLen > 0) {
        editor
          .chain()
          .focus()
          .deleteRange({ from: $from.pos - matchLen, to: $from.pos })
          .run();
      }
      setSlashOpen(false);
      if (cmd.id === "image") {
  ...
  const insertImageFromUrl = useCallback(() => {
    ...
    editor
      .chain()
      .focus()
      .setImage({ src: imageInputUrl.trim(), alt: imageAlt || undefined })
      .run();
  ...
  /* Save */
  const doSave = async (published: boolean, scheduledDate?: string) => {
    if (!title.trim()) {
  ...
  return (
    <div className="w-full">
        ...
        onChange={(e) => {
          ...
          if (!postId)
            triggerAutoSave(
              e.target.value,
              subtitle,
              (editor?.getJSON() as unknown as string) ?? "",
            );
        }}
      ...
      <div className="mt-8 rounded-xl border border-border bg-card px-4 py-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tags.map((t) => (
            <span
              key={t}
              className="flex items-center gap-1 rounded-full bg-muted px-2.5 py-0.5 text-sm text-foreground"
            >
              {t}
              <button
          ...
          <input
            ...
            placeholder={
              tags.length < 10 ? "Add tags (Enter or comma)…" : "Max 10 tags"
            }
      ...
      <div className="mt-6 flex items-center justify-end gap-3">
        ...
        <Button variant="outline" onClick={() => setOpenScheduledModal(true)}>
          Scheduled Post
        </Button>
        <Button
          onClick={() => doSave(true)}
        ...
        {/* Modal */}
        <ScheduledPostModal
          onSchedulePost={async (scheduledDate) => {
            // wait for the post to save
            await doSave(false, scheduledDate);
            setOpenScheduledModal(false);
          }}
          open={openScheduledModal}
          schedulingPost={!!saving}
          setOpenModal={setOpenScheduledModal}
        />
      </div>
    </div>
  );
